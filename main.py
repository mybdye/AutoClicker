...
class Runthread(QtCore.QObject):
    ...
    def run(self):
        ...
        print('>>> run Start')
        self.signal.emit([counter, '>>> run Start'])
        if gv.get_value('save') == 1:
            counter = gv.get_value('lineEdit_counter')
            gv.set_value('save', 0)
        for i in range(0, waitFirstTime):
        ...
        while self.flag:
            if counter > 0:
                ...
                if last_mouse_position == current_mouse_position:
                    ...
                    self.signal.emit([counter, '剩余次数：'+ str(counter)+ '\n'+ str(pyautogui.position())])
                else:
                    print('剩余次数：'+ str(counter) + ' 鼠标移动中!')
                    self.signal.emit([counter, '剩余次数：'+ str(counter) + ' 鼠标移动中!'])
                continue
            else:
                self.signal.emit([0, '>>> run End'])
                break
